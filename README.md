# WonderGo 玩得夠

互動式第一期產品原型。

## 啟動

```bash
python3 -m http.server 4173
```

瀏覽器開啟 `http://localhost:4173`。

## 示範帳號

- 玩家：`player` / `Wonder1234`
- 教師：`teacher` / `Wonder1234`

也可以從登入頁建立新玩家或教師帳號。原型資料儲存在瀏覽器 `localStorage`。

## 已完成

- 玩家／教師雙入口
- 玩家與教師首次註冊
- 帳號密碼再次登入
- 玩家冒險首頁
- 五大能力訓練館
- 可操作示範題與 XP 累積
- 玩家能力儀表板及本週趨勢
- 托奇週報
- 教師班級總覽
- 個別學生學習分析
- 響應式手機、平板及桌面版面

## 正式版下一步

## 連接 Supabase 雲端

WonderGo 已連接 Supabase 專案 `Charmingbei's Project`。

- 資料庫結構及 RLS：`supabase/schema.sql`
- 前端連線設定：`cloud-config.js`
- 雲端資料層：`cloud.js`
- 無 Email 學生註冊：Supabase Edge Function `register-wondergo-user`

重新整理網站後，登入頁顯示「雲端資料庫已連線」即代表成功。

`anon` key 可以放在前端，真正的資料保護由 `schema.sql` 內的 Row Level Security 政策負責。請勿將 `service_role` key 放入 `cloud-config.js`。

雲端版目前同步：

- 玩家與教師註冊、登入和基本資料
- 玩家帳號同時作為遊戲內公開名稱
- 地圖式五大能力訓練館
- 每座場館依 CEFR 程度每日安排 10 題國小英語練習
- 題組依台北日期與學生帳號每日更新，同日保持一致
- A1 以上混合本級新題與基礎複習題，Pre-A1 維持基礎範圍
- 三組每日推薦任務分別訓練閱讀、聽力與拼字
- 同一場館或推薦任務當日第一次提供完整 XP，第二次起 XP 折半
- 世界航圖包含十個主題國家，每國提供 10 個圖像單字與聽、說、讀、寫關卡
- 主題國家單字支援點擊發音，每座關卡完成後可獲得 XP
- 主題拼字工坊使用完整英文單字作為選項，不顯示缺字或亂序字串
- XP 與答題學習事件
- 玩家「我的能力」依個人實際五項進度及本週學習事件呈現
- 完成訓練後即時更新能力雷達圖、能力值與個人化分析
- 管理員查看所有學生、教師、待審核申請及班級配對
- 管理員可核准教師帳號，並查看各班教師、學生及五大能力
- 管理員帳號頁可依全部、教師、學生篩選
- 管理員可補登教師信箱，並修改姓名、學校、班級、座號、程度與核准狀態
- 教師信箱修改會同步更新登入及忘記密碼收件信箱
- 密碼僅顯示遮罩與最近更新時間，系統不儲存或揭露明文密碼
- 教師後台僅讀取自己班級的真實學生資料
- 教師查看全班與個別學習狀況、關注學生、易錯題型及單字
- 教師教材管理：建立、編輯、發布及封存結構化題組
- 教材題目支援逐題卡片編輯、新增、刪除、排序，以及多題批次貼上匯入
- 教師任務指派：選擇已發布教材並指派全班、個別學生或自選多人小組
- 學生首頁即時顯示教師任務、截止日期、XP 獎勵及完成狀態
- 指派任務完成後安全發放一次 XP，並回傳教師端完成紀錄
- 新教師註冊需填信箱，並可由登入頁寄送安全密碼重設連結
- 每題作答紀錄用於班級錯題分析
- 教師任務資料結構

新註冊教師預設為待核准，避免冒用教師身分查看兒少資料。可在 Supabase `profiles` 表將該教師的 `is_approved` 設為 `true`。

管理員不開放前台註冊。先建立教師帳號，再於 Supabase 將該帳號的
`role` 改為 `admin`、`is_approved` 改為 `true`，之後從「教師入口」登入。

後續可再串接口說錄音 Storage、語音評分服務及完整教材編輯器。

## GitHub Pages

專案包含 `.github/workflows/deploy-pages.yml`。推送至 GitHub 的 `main` 分支後，GitHub Actions 會自動發布網站。

首次使用時，在 repository 的 **Settings → Pages → Build and deployment** 將來源設為 **GitHub Actions**。
